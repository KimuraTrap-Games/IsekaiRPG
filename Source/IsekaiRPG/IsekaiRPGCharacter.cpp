...
void AIsekaiRPGCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent)) {
		...
		// Moving
		EnhancedInputComponent->BindAction(MoveAction, ETriggerEvent::Triggered, this, &AIsekaiRPGCharacter::Move);
		EnhancedInputComponent->BindAction(MouseLookAction, ETriggerEvent::Triggered, this, &AIsekaiRPGCharacter::Look);
	...
	{
		UE_LOG(LogIsekaiRPG, Error, TEXT("'%s' Failed to find an Enhanced Input component! This template is built to use the Enhanced Input system. If you intend to use the legacy system, then you will need to update this C++ file."), *GetNameSafe(this));
	}
}

void AIsekaiRPGCharacter::Move(const FInputActionValue& Value)
{
	// input is a Vector2D
...
void AIsekaiRPGCharacter::Look(const FInputActionValue& Value)
{
	// input is a Vector2D
...
void AIsekaiRPGCharacter::DoMove(float Right, float Forward)
{
...
void AIsekaiRPGCharacter::DoLook(float Yaw, float Pitch)
{
	if (GetController() != nullptr)
	{
...
void AIsekaiRPGCharacter::DoJumpStart()
{
	// signal the character to jump
	Jump();
}

void AIsekaiRPGCharacter::DoJumpEnd()
{
